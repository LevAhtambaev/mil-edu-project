...
file_path="RLS2/detected_Rls2.txt" # обнаруженные цели РЛС.
file2_path="RLS2/confrimed_Rls2.txt" # подтвержденные цели РЛС.
...
echo "$time,Rls2,i am alive" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "messages/alive/$time.log"
...
if [ -d "$directory" ]; then # проверка, является ли путь директорией
   ...
   while true; do
      ...
      for name in "${filenames[@]}"; do
         ...
         sector=$(target_in_sector $x $y $RLS2_x $RLS2_y $RLS2_RADIUS $RLS2_START_ANGLE $RLS2_END_ANGLE) # Находится ли цель в секторе обзора РЛС.
            ...
            if ! grep -q "^$id " "$file2_path"; then # Если цель записана в файле confrimed_targets, то мы её игнорируем.
               if grep -q "^$id " "$file_path"; then # Если цель уже встретилась один раз и записана в detected_targets, то считаем её скорость и записываем в confrimed.
                  ...
                  if [[ -n $speed_info && $speed_info != 0 ]]; then # Есть ли скорость, неравная нулю.
                     ...
                     echo "$time,Rls2,confirm,$id,$x $y,$speed_info,$type" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "$messages_path/$time.log" # Запись информации о подтвержденной цели в файл журнала сообщений.
                     ...
                     if [[ $to_spro == "true" && $type == "ballistic" && $(echo "${distance1} < ${distance2}" | bc -l) ]]; then
                        ...
                        echo "$time,Rls2,toSpro,$id,$x $y" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "$messages_path/$time.log" # Запись этого сообщения в файл журнала сообщений.
                     fi
                     speed_info=0
                  fi
               else
      ...
      if [ -f "RLS2/hello.log" ]; then
            decrypted_content=$(openssl aes-256-cbc -pbkdf2 -a -d -salt -pass "pass:$password" -in "RLS2/hello.log")
            if [ "$decrypted_content" = "hello" ]; then
               time=$(TZ=Europe/Moscow date +"$time_format")
                echo "$time,Rls2,i am alive" | openssl aes-256-cbc -pbkdf2 -a -salt -pass pass:$password > "messages/alive/$time.log" # Сообщение о том, что РЛС жива, и запись в файл.
            fi
            rm "RLS2/hello.log"
      fi
   done
